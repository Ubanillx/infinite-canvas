...
+ [修复] AI 代理安全处理压缩响应和中途断流，网络失败不再误报跨域，单图重试固定请求一张，服务重启保留诊断日志。

+ [修复] WebDAV 工作区目录初始化允许安全探测和创建共享根目录，并区分工作区认证、路径拒绝与上游账号认证错误。
+ [修复] WebDAV 并发创建同一快照目录时按目录级加锁并二次确认，避免群晖竞争状态被误报为认证失败；快照列表兼容 nanoid 标识。
+ [修复] WebDAV 同源代理不再重复拼接服务器配置的基础目录，避免工作区同步请求落到错误远端路径并返回 404。
+ [修复] WebDAV 连接测试忽略旧配置中的基础目录字段，避免升级后测试请求误访问错误路径。
+ [新增] 网页首次进入时可创建或解锁受访问密钥保护的工作区；画布、素材、工作台记录与本地媒体按工作区隔离，首个新工作区会一次性迁移旧浏览器数据。
+ [调整] WebDAV 自动同步改为仅上传当前工作区快照，远端历史必须由用户手动选择后导入；导入按记录 ID 合并并优先保留本机内容，每个工作区保留最近 30 个快照。
+ [安全] WebDAV 代理、Canvas Agent 代理和直接加载的 Agent 附件均要求当前工作区凭据，远端路径限制为对应工作区目录。
+ [修复] Canvas Codex Agent 的会话、SSE 连接、画布状态、技能、审批、对话补充历史和消息附件元数据均按工作区分离；AI 渠道与 API Key 继续作为服务器全局配置。
+ [新增] 供应商新增自定义视频 API 协议，支持通过 Bun 服务端代理调用 Seedance 与 Grok 视频模型；只写 API Key 持久化到用户目录并脱离 Git 跟踪。
...
+ [修复] 本地存储页在非安全 HTTP 环境无法使用浏览器容量估算接口时不再报错。
+ [新增] 新增后台构建并平滑重启前端预览服务的运维脚本。
+ [修复] Image2 生成结果加入资产时复用本地存储，GPT Image 的 R2 图片通过同源代理下载，避免远程地址过期或跨域导致保存失败。
+ [修复] 群晖 WebDAV 通过固定目标的同源代理连接，避免浏览器跨域预检失败。
+ [调整] AI 渠道、生成偏好、提示词来源和 WebDAV 配置改由 Bun 服务端持久化，并支持首次自动迁移浏览器配置。
+ [新增] WebDAV 支持自动同步敏感配置、画布、资产、生成记录及媒体文件，并提供周期、状态与失败提示。
+ [修复] WebDAV 目录检查改为先确认是否存在并合并并发请求，避免对已有群晖目录重复发送 `MKCOL` 产生 405。
+ [安全] 服务端配置接口不再返回 AI API Key 和 WebDAV 密码；敏感字段改为只写替换，前端不可二次查看。
+ [新增] Canvas Codex Agent 支持服务端同源代理、自动连接和 Web/Agent 一键后台启停。
+ [优化] Agent 面板打开时直接进入对话，连接信息改为通过状态入口按需查看。
+ [修复] WebDAV 认证失败不再触发浏览器原生登录框，改由网页显示错误提示。
+ [安全] WebDAV 请求统一改由 Bun 同源代理使用服务端密钥，浏览器不再直连群晖、携带认证头或持久化 WebDAV 密码。
+ [修复] 新浏览器通过服务端保存的渠道 ID 调用 AI 生成、编辑和模型列表，API Key 继续只保存在 Bun 服务端，无需重新导入配置。
+ [安全] 服务端敏感接口限制在配置的内网网段，AI 代理拒绝本地/链路本地上游地址；浏览器本地存储和 Agent 构建产物不再保存 API Key 或 Agent token。
