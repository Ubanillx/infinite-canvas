export default {
    ...
    config: {
        ...
        serverStorage: {
            loading: "正在加载服务器配置",
            loadFailed: "无法加载服务器配置",
            saveFailed: "服务器配置保存失败",
            invalidResponse: "服务器返回的配置格式不正确",
            requestFailed: "配置服务器请求失败（{{status}}）",
            upload: "上传到服务器",
            uploaded: "配置已上传到服务器",
        },
        tabs: {
        ...
        fileSecurity: "配置只从服务器加载并保存在当前会话内存中；修改后请点击“上传到服务器”才能共享给其他人。API Key 与 WebDAV 连接信息不会写入导出的 JSON 文件。",
        import: "导入配置",
        export: "导出配置",
        imported: "配置与用户偏好已导入当前会话（尚未上传服务器）",
        importedDirectConfig: "已导入本地直连配置",
        importFailed: "配置文件读取失败",
        saved: "配置已应用到当前会话（尚未上传服务器）",
        savedContinue: "配置已应用到当前会话（尚未上传服务器），请继续刚才的请求",
        channels: {
        ...
        channelEditor: {
            ...
            protocol: "协议",
            baseUrl: "接口地址",
            apiKeyConfigured: "已设置；输入新密钥可替换",
            apiKeyPlaceholder: "输入 API Key",
            apiKeyWriteOnly: "密钥只允许修改，保存后不会再次显示。",
            models: "渠道模型",
        ...
        webdav: {
            title: "WebDAV 同步",
            description: "配置从服务器加载；需要同步工作区数据时，请手动点击“立即同步”。",
            serverManagedTitle: "服务器托管连接",
            serverManagedDescription: "WebDAV 地址、目录和登录凭据已保存在服务器上，浏览器不会读取或显示这些信息。",
            lastSynced: "上次同步 {{time}}",
            configLastSynced: "配置同步 {{time}}",
            neverSynced: "尚未同步",
            url: "WebDAV 地址",
            directory: "远程目录",
            ...
            username: "用户名",
            password: "密码 / 应用密码",
            passwordConfigured: "已设置；输入新密码可替换",
            passwordPlaceholder: "输入密码 / 应用密码",
            passwordWriteOnly: "密码只允许修改，保存后不会再次显示。",
            autoSync: "自动同步（已停用）",
            autoSyncDescription: "当前所有用户均采用手动同步，配置只在点击“上传到服务器”后写入服务器。",
            autoSyncInterval: "全量同步周期",
            intervalMinutes: "每 {{count}} 分钟",
            sensitiveConfigTitle: "远端配置包含敏感信息",
            sensitiveConfigDescription: "WebDAV 的 config.json 包含 AI API Key 和 WebDAV 凭据，请确保远端目录仅可信账号可访问。",
            ...
            errors: { testFailed: "WebDAV 连接测试失败", downloadFailed: "读取 WebDAV 同步文件失败", downloadTimeout: "读取 WebDAV 同步文件超时", emptyUpload: "上传文件为空，已取消上传", uploadFailed: "上传 WebDAV 同步文件失败", directoryFailed: "创建 WebDAV 远程目录失败", requestTimeout: "WebDAV 请求超时，请检查网络或远端服务状态", connectionFailed: "无法连接 WebDAV，请检查地址、HTTPS 证书、CORS 或网络状态", urlRequired: "请先填写 WebDAV 地址", authenticationFailed: "WebDAV 认证失败，请检查用户名、密码或应用密码", workspaceAuthenticationFailed: "工作区认证已失效，请重新选择并解锁工作区", workspacePathNotAllowed: "当前工作区不允许访问该 WebDAV 路径", pathMissing: "WebDAV 路径不存在，请检查地址或服务器端远程目录", responseFailed: "{{fallback}}：{{status}}{{detail}}", syncFailed: "同步失败", invalidManifest: "{{domain}} 同步清单不是当前应用的数据" },
        },
        protocols: {
            ark: "火山方舟",
            customVideo: "自定义视频 API",
        },
    },
    agent: {
