...
The OpenAI-compatible flow uses `POST /v1/videos`, `GET /v1/videos/{id}`, and `GET /v1/videos/{id}/content`. After a remote task is created, its ID is stored on the video node so a page refresh can keep polling, or you can click **Get task status**. Polling stops when the video is ready, the task fails, or the status request errors. For Volcengine Ark Agent Plan and Seedance 2.0, use `https://ark.cn-beijing.volces.com/api/plan/v3` and enter the model name manually because this endpoint does not expose OpenAI's `/models` API.
